...
ROWS = [
    ("host",),
    ("kv", "Now", "Freelancer"),
    ("kv", "Prev", "Full-Stack Developer (Projects)"),
    ("kv", "Edu", "B.Tech CS, LPU '27 · 8.19 CGPA"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Lang", "C++, JavaScript, TypeScript, SQL"),
    ("kv", "Frontend", "React, Next.js, Tailwind CSS"),
    ("kv", "Backend", "Node, Express, Laravel"),
    ("kv", "Database", "PostgreSQL, MongoDB, Supabase"),
    ("kv", "Cloud", "Vercel, Supabase, Docker"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "CareerPilot — AI career coach (React + Laravel)"),
    ("bul", "WealthIQ.ai — AI finance SaaS (Next.js)"),
    ("bul", "300+ DSA problems on LeetCode & HackerRank"),
    ("gap",),
    ("sec", "Certs"),
    ("bul", "OCI Generative AI Professional"),
    ("bul", "HackerRank SQL Certified"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">raj@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">raj</tspan><tspan fill="{MUTED}">@</tspan>'
